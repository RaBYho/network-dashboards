  ...
  <div class="flex flex-col h-full bg-gray-50">
    ...
    <div class="flex items-center justify-between px-5 py-3 bg-white border-b border-gray-200">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-medium text-gray-800">Erreurs & anomalies</span>
      </div>
      <div class="flex items-center gap-2">
        <button
    ...
    <div class="flex-1 p-4 flex flex-col gap-3 overflow-auto">
      ...
      <div class="grid grid-cols-4 gap-3">
        ...
        <KpiCard
          ...
          sub="eth0 + wlan0"
          icon="ti-check"
          :value-class="parseFloat(avgErrorRate) > store.thresholds.error_rate_pct
      ...
      <div class="bg-white border border-gray-200 rounded-lg p-4">
        <div class="flex items-center justify-between mb-3">
          ...
          <div class="flex gap-3 text-xs text-gray-400">
            <span><span class="inline-block w-2 h-2 rounded-sm bg-red-500 align-middle mr-1"></span>Critique</span>
            <span><span class="inline-block w-2 h-2 rounded-sm bg-amber-400 align-middle mr-1"></span>Avertissement</span>
          </div>
        </div>
        <apexchart
          type="bar"
      ...
      <div class="flex-1 bg-white border border-gray-200 rounded-lg p-4">
          ...
          <table class="w-full text-xs border-collapse">
            <thead>
              <tr class="border-b border-gray-100 bg-gray-50">
                <th class="text-left py-2 px-3 font-medium text-gray-400 w-32">Horodatage</th>
                <th class="text-left py-2 px-3 font-medium text-gray-400 w-24">Sévérité</th>
                <th class="text-left py-2 px-3 font-medium text-gray-400 w-20">Interface</th>
                <th class="text-left py-2 px-3 font-medium text-gray-400">Message</th>
        ...
        <div class="flex items-center justify-between pt-3 border-t border-gray-100 mt-2">
          <span class="text-xs text-gray-400">
            Affichage {{ paginationStart }}–{{ paginationEnd }} sur {{ filteredErrors.length }} erreurs
          </span>
...
const severityFilters = [
  { value: 'all',      label: 'Tous',          activeClass: 'bg-gray-100 text-gray-700 border-gray-300'    },
  { value: 'critical', label: 'Critique',       activeClass: 'bg-red-50 text-red-700 border-red-300'        },
  { value: 'warn',     label: 'Avertissement',  activeClass: 'bg-amber-50 text-amber-700 border-amber-300'  },
  { value: 'info',     label: 'Info',           activeClass: 'bg-blue-50 text-blue-700 border-blue-300'     },
]
...
const filteredErrors = computed(() => {
  return store.errors.filter(e => {
    const matchSev   = activeSeverity.value === 'all' || e.severity === activeSeverity.value
    const matchIface = activeIface.value === 'all'    || e.iface === activeIface.value
...
// ── Pagination ───────────────────────────────────────
const totalPages     = computed(() => Math.max(1, Math.ceil(filteredErrors.value.length / perPage)))
const paginationStart = computed(() => filteredErrors.value.length === 0 ? 0 : (currentPage.value - 1) * perPage + 1)
const paginationEnd   = computed(() => Math.min(currentPage.value * perPage, filteredErrors.value.length))
const paginatedErrors = computed(() =>
  filteredErrors.value.slice((currentPage.value - 1) * perPage, currentPage.value * perPage)
)

// ── KPI ──────────────────────────────────────────────
const criticalCount   = computed(() => store.errors.filter(e => e.severity === 'critical').length)
const warnCount       = computed(() => store.errors.filter(e => e.severity === 'warn').length)
const totalPacketLoss = computed(() =>
  Math.round((store.latency.eth0.packet_loss_pct + store.latency.wlan0.packet_loss_pct) * 10)
)
const avgErrorRate = computed(() =>
  ((store.latency.eth0.packet_loss_pct + store.latency.wlan0.packet_loss_pct) / 2).toFixed(1)
)
...
function ifaceClass(i) {
  if (i === 'eth0')  return 'bg-purple-50 text-purple-700'
  if (i === 'wlan0') return 'bg-green-50 text-green-700'
...
const histoOptions = computed(() => ({
  ...
  xaxis: {
    categories: ['-60m','-55m','-50m','-45m','-40m','-35m','-30m','-25m','-20m','-15m','-10m','-5m','now'],
